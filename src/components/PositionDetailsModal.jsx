...
const PositionDetailsModal = ({ position, allTransactions = [], currentLang = 'en', selectedCurrency = 'usd', onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 max-w-lg w-full shadow-2xl space-y-4 max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">
                {currentLang === 'pt' ? 'Detalhes da Posição & Histórico DCA' : 'Position Details & DCA History'}
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="overflow-y-auto space-y-4 pr-1 scrollbar-thin scrollbar-thumb-slate-700">
          ...
          {/* Card de Resumo do Preço Médio (DCA) */}
          <div className="bg-slate-800/80 rounded-xl p-3.5 border border-slate-700 space-y-2">
            <span className="text-[10px] text-amber-400 font-bold uppercase tracking-wider block">
              📊 {currentLang === 'pt' ? 'Resumo de Preço Médio Ponderado (DCA)' : 'Weighted Average Price Summary (DCA)'}
            </span>
